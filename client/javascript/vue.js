...
var app2 = new Vue({
  ...
  methods: {
    ...
    newUserRegister() {
      axios({
        method: 'post',
        url: 'http://34.87.97.143/users/signup',
        data: {
    ...
    userLogin() {
      axios({
        method: 'post',
        url: 'http://34.87.97.143/users/signin',
        data: {
          email: this.email,
          password: this.password
        }
    ...
    createArticle() {
      ...
      axios({
        method: 'post',
        url: 'http://34.87.97.143/articles/add',
        data: formData,
    ...
    readArticle() {
      axios({
        method: 'get',
        url: 'http://34.87.97.143/articles/read',
        headers: { token: localStorage.getItem('token') }
      })
        .then(({ data }) => {
    ...
    deleteArticle(id) {
      ...
      axios({
        method: 'delete',
        url: `http://34.87.97.143/articles/delete/${id}`,
        headers: { token: localStorage.getItem('token') }
      })
        .then(({ data }) => {
          console.log(data);
    ...
    updateArticle() {
      ...
      axios({
        method: 'patch',
        url: `http://34.87.97.143/articles/edit/${this.id}`,
        data: formData,
        headers: {
    ...
    allArticles() {
      axios({
        method: 'get',
        url: `http://34.87.97.143/articles/all`,
        headers: {
          token: localStorage.getItem('token')
        }
      })
    ...
    onSignIn(user) {
      ...
      axios({
        url: `http://34.87.97.143/users/googleSignin`,
        method: "post",
        headers: {
          token: id_token
        }
